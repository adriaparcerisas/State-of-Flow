...
prices AS (
  SELECT dateadd(ms, value[0], to_timestamp('1970-01-01'))::date AS date, value[1]::float AS price
  FROM api_data, LATERAL FLATTEN(input => resp:data:prices)
),
market_caps AS (
  SELECT dateadd(ms, value[0], to_timestamp('1970-01-01'))::date AS date, value[1]::float AS market_cap
  FROM api_data, LATERAL FLATTEN(input => resp:data:market_caps)
),
total_volumes AS (
  SELECT dateadd(ms, value[0], to_timestamp('1970-01-01'))::date AS date, value[1]::float AS volume
...
total_supply_data AS (
  SELECT date, (market_cap / NULLIF(price,0))::float AS total_supply
  FROM parsed_data
),
llama_api AS (
  SELECT livequery.live.udf_api('https://api.llama.fi/v2/historicalChainTvl/flow') AS resp
),
chain_tvl AS (
  SELECT dateadd('second', value:date::int, to_timestamp('1970-01-01'))::date AS date,
         value:tvl::float AS protocol_locked
  FROM llama_api, LATERAL FLATTEN(input => resp:data)
),
staking_data AS (
  WITH staking AS (
...
supply_breakdown AS (
  SELECT
    ...
    MAX(sd.total_staked_volume)                            AS staked_locked,          -- native staking (on-chain)
    MAX(ct.protocol_locked)                                AS non_staked_locked,      -- total protocol TVL (DeFiLlama)
    0.0                                                    AS liquid_supply           -- not available from API fallback
  FROM parsed_data p
  LEFT JOIN total_supply_data ts ON p.date = ts.date
  LEFT JOIN chain_tvl        ct ON p.date = ct.date
  LEFT JOIN staking_data     sd ON p.date = sd.date
  GROUP BY p.date
),
final AS (
  SELECT
    ...
    staked_locked,
    non_staked_locked,
    GREATEST(total_supply - (staked_locked + non_staked_locked + liquid_supply), 0) AS unstaked_circulating,
    liquid_supply,
    (total_supply - GREATEST(total_supply - (staked_locked + non_staked_locked + liquid_supply), 0)) AS free_circulating_supply
